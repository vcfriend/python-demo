...
class TrailStrategy(bt.Strategy):
    ...
    def next(self):
        print(
            f"{self.datetime.date()}: Open: {round(self.data.open[0], 2)}, High: {round(self.data.high[0], 2)}, Low: {round(self.data.low[0], 2)}, Close: {round(self.data.close[0], 2)}")
        if not self.position:
            if self.crup:
                o = self.buy()
                self.order = None
                print(
                    colorama.Style.BRIGHT + colorama.Fore.GREEN + ' BUY  ' + colorama.Fore.RESET + colorama.Style.RESET_ALL,
                    end='')
            ...
            self.order = self.sell(exectype=bt.Order.StopTrail, trailamount=self.p.trailamount,
            ...
            print(
                colorama.Fore.RED + f"{self.datetime.date()}: Close: {round(self.data.close[0], 2)}, Trigger Price: {round(self.order.created.price, 2)}, Ideal Trigger Price: {round(trigger_price, 2)}" + colorama.Fore.RESET)
        else:
            if self.p.trailamount:
                trigger_price = self.data.close - self.p.trailamount
            else:
    ...
    def notify_order(self, order):
        ...
        if order.status == order.Completed:
            print('-- Notify -- {}: Created: {} Type {} Price: {} Size: {}'.format(self.data.datetime.date(0),
