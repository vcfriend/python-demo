...
def parse_args(pargs=None):
    ...
    parser.add_argument('--fromdate', required=False, default='20180102',
                        help='Starting date in `dtformat` format')
    parser.add_argument('--todate', required=False, default='20190202',
...
class TestStrategy(bt.Strategy):
    ...
    def log(self, txt, dt=None):
        ...
        # 时间断点调试,调试条件 self.datas[0].datetime.datetime(0) >= bt.datetime.datetime.strptime('2018-01-03 13:15:00','%Y-%m-%d %H:%M:%S')
    ...
    def order_target(self, size=1):
        ...
        # 按目标金额下单
            ...
            # value = (size / 100.0) * self.broker.getvalue()  # 帐户总资金百分比交易
            value = (size / 100.0) * self.broker.getcash()  # 可用资金百分比交易
    ...
    def next(self):
        ...
        if not self.position and self.position.size == 0 and (assets > 0):
            ...
            if self.sig_long:  # 买入
                ...
                self.positionflag = -1
                # self.myorder = self.order_target_percent(target=-abs(self.mposkk / 100))  # 百分比开仓
                self.mposkk = -abs(self.mpposmin*1)
                self.myorder = self.order_target(self.mposkk)
                self.radd = self.myentryprice * (1 - self.mpr)
                self.sexit = self.myentryprice * (1 + self.mps)
