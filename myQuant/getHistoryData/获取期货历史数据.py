...
if "__main__" == __name__:
    ...
    # 随便选了 1 个期货来测试，
    secid = secids[94]  # 合约id
    # secid = '114.cm'  # 合约id
    klt = 15  # klt=5为5分钟周期，klt=101为日周期
    beg = '20150101'  # 开始时间
    end = '2021119'  # 结束时间
    # 随便选的期货名称
    futures_name = futures_names[94]
    print(f'正在获取期货：{futures_name} 的历史 k 线数据')
    # 获取期货历史 日k 线数据
    # secid 可以在文件 期货信息表.csv 中查看，更改 secid 即可获取不同的期货数据, klt=5为5分钟周期，klt=101为日周期
    df = get_ftures_k_history(secid, klt=klt)
    df.insert(0, '期货名称', [futures_name for _ in range(len(df))])
    df.to_csv(f'{futures_name}-{klt}.csv', index=None, encoding='utf-8-sig')
    print(f'期货 : {futures_name} {klt}周期的 k 线数据已保存到文件 {futures_name}-{klt}.csv 中')