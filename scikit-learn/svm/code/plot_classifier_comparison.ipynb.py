...
X, y = make_classification(
    ...
    flip_y=0.1,  # (default=0.01)类别随机分配的样本比例。 较大的值会在标签中引入噪音，并使分类任务更加困难。
...
rng = np.random.RandomState(
    2)  # 使用RandomState(int)可以自定义随机数种子，以保证每次执行代码，生成的数组都相同。 https://blog.csdn.net/weixin_42782150/article/details/102841192
...
datasets = [
    ...
    make_moons(n_samples=n_samples,
               ...
               noise=0.1,  # double or None (default=None)加到数据中的高斯噪声的标准偏差。
    ...
    make_circles(n_samples=n_samples,
                 # int or two-element tuple, optional (default=100)如果为int，则为生成的总点数。 对于奇数，内圆比外圆多一个点。如果是两个元素的元组，则是在外圆和内圆中的点数。
                 noise=0.1,  # double or None (default=None)加到数据中的高斯噪声的标准偏差。
                 shuffle=True,  # bool, optional (default=True)是否打乱数据顺序。
    ...
    for name, clf in zip(names, classifiers):
        ...
        if hasattr(clf, "decision_function"):  # 检查对象是否拥有决策函数,
            ...
            Z = clf.predict_proba(np.c_[xx.ravel(), yy.ravel()])[:,
                1]  # 线性可分的,类别概率 https://blog.csdn.net/skyejy/article/details/90664763
