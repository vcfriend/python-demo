...
for ds_cnt, ds in enumerate(datasets):
    ...
    for name, clf in zip(names, classifiers):
        ...
        score1 = clf.score(X_train, y_train)  # 训练集R2评分系数
        score = clf.score(X_test, y_test)  # 测试集R2评分系数

        # Plot the decision boundary. For that, we will assign a color to each
        # 绘制决策边界。为此，我们将为每一个分配一个颜色
        # 网格中的点 [x_min, x_max]x[y_min, y_max].
        if hasattr(clf, "decision_function"):  # 检查对象是否拥有某个属性
        ...
        ax.text(
            xx.max() - 0.3,
            yy.min() + 0.3,
            ("%.2fs %.2f %.2f" % ((time_end - time_begin), score1, score)),
            size=15,
